...
std::string Clerk::Get(std::string key)
{
    ...
    raftKVRpcProctoc::GetArgs args; // [Key, ClientID, RequestID]
    args.set_key(key);
    ...
    while (true)
    {
        raftKVRpcProctoc::GetReply reply;   // [Err, Value]
        bool ok = m_servers[server]->Get(&args, &reply);

        // 失败会一直重试，因为requestId没有改变，因此可能会因为RPC的丢失或者其他情况导致重试，kvserver层来保证不重复执行（线性一致性）
