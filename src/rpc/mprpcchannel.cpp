...
/**
 * @brief 所有通过 rpc stub代理对象调用的rpc方法，都会被转发到这里，【发送rpc请求，并接收响应】
                              ...
                              google::protobuf::Message *response, google::protobuf::Closure *done)
{
    ...
    // 序列化request，并获取其长度args_size
    uint32_t args_size{};
    std::string args_str;   // 存储request序列化结果
    ...
    std::string send_rpc_str; // 用来存储最终发送的数据
    {
        ...
        // CodedOutputStream(处理二进制编码的一种流输出类) = 变长编码(减少数据大小) + 高效处理
    ...
    // 发送rpc请求 - 失败会重试连接再发送，重试连接失败会直接return
